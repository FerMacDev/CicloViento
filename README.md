...
La consulta explícita `GET /route-plans/:id/weather` usa Open-Meteo Forecast API sin API key para este uso. Muestra condición general mediante iconos locales, temperatura, sensación térmica, probabilidad de precipitación, velocidad, rachas, dirección meteorológica y nivel de riesgo. Sin una hora en RoutePlan, utiliza las 09:00 locales como referencia provisional. La interfaz atribuye los datos a Open-Meteo.
...
Al activar **Ruta favorable al viento**, el backend compara hasta tres recorridos circulares reales con las semillas deterministas 1, 2 y 3. El margen ±20 % de distancia es preferente: si hay alternativas dentro de él, selecciona la de mejor viento para el regreso (`wind-optimized`); en empate prefiere la distancia más próxima y después la semilla menor. Si ninguna entra en el margen, devuelve la ruta real más cercana como `distance-fallback`, avisando de que no se considera optimizada por distancia. Nunca se inventa una ruta; si ORS no devuelve ninguna geometría real, se muestra un error controlado. La previsión se consulta una vez para el punto de salida y puede cambiar. Favorabilidad no equivale a seguridad: la interfaz advierte ante riesgo alto o peligroso y no presenta esas condiciones como seguras. Aún no hay optimización avanzada, hora de salida configurable ni IA.
