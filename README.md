...
El botón **Generar recorrido** llama de forma explícita a `POST /route-plans/:id/generate`. El backend usa openrouteservice Directions v2 con perfil `cycling-road`, round-trip y respuesta GeoJSON; la clave `ORS_API_KEY` se configura exclusivamente en `backend/.env` y nunca se expone al frontend. La API pública de ORS limita los recorridos circulares a 100 km: no se genera ni trunca una ruta superior. Si ORS confirma que no puede generar el recorrido circular, CicloViento intenta como alternativa una ruta real de ida y vuelta: calcula por separado la ida y el regreso de carretera hacia uno de hasta tres destinos deterministas. No invierte una geometría ni inventa puntos; un fallo técnico, timeout, cuota o respuesta inválida se devuelve como error y no activa este fallback. El mapa muestra la polyline real devuelta y distingue la distancia solicitada de la generada. Al solicitar una ruta favorable al viento, el backend compara hasta tres candidatas antes de devolver una sola geometría seleccionada.
...
La ruta y el viento se muestran en el mismo mapa Leaflet. Cuando ya existe una previsión, una capa activable coloca siete flechas sobre puntos reales del recorrido para indicar hacia dónde sopla el viento: una dirección meteorológica del oeste (`270°`) se representa hacia el este (`90°`). El color reutiliza el nivel de riesgo existente: verde (`normal`), ámbar (`caution`), naranja (`high`) o rojo (`dangerous`), y la leyenda lo explica también en texto. Las flechas permanecen ancladas al mapa durante el zoom y el desplazamiento. No se muestran si no hay previsión disponible y no existe un segundo mapa meteorológico. Las flechas informan de la dirección del viento, no recomiendan el sentido para recorrer el circuito: esa decisión sigue correspondiendo al ciclista. Como mejora futura, el viento podrá variar por tramos según la duración estimada y los pronósticos horarios.
...
Al activar **Ruta favorable al viento**, el backend compara hasta tres recorridos circulares reales con las semillas deterministas 1, 2 y 3. El margen ±20 % de distancia es preferente: si hay alternativas dentro de él, selecciona la de mejor viento para el regreso (`wind-optimized`); en empate prefiere la distancia más próxima y después la semilla menor. Si ninguna entra en el margen, devuelve la ruta real más cercana como `distance-fallback`, avisando de que no se considera optimizada por distancia. Cuando ningún circuito existe, se aplica el fallback real de ida y vuelta y, si hay previsión, se analiza su geometría; el resultado se identifica como `routeKind=out-and-back`, sin `selectionMode` ni afirmación de optimización por viento. Si todavía no existe previsión para la fecha y hora solicitadas, se genera una única ruta real normal y se marca como `weather-unavailable`, sin score ni análisis inventados; mapa y GPX siguen disponibles. Nunca se inventa una ruta; si ORS no devuelve ninguna geometría real, se muestra un error controlado. Favorabilidad no equivale a seguridad: la interfaz advierte ante riesgo alto o peligroso y no presenta esas condiciones como seguras. Aún no hay optimización avanzada ni IA.

## Descarga GPX

Cuando hay una ruta generada, el botón **Descargar GPX** solicita `GET /route-plans/:id/gpx` con la sesión autenticada. El backend devuelve un archivo GPX 1.1 con la geometría real de la ruta mostrada en el mapa, sin inventar elevaciones, datos personales ni secretos. Para una ruta favorable al viento, reutiliza la candidata seleccionada por la misma lógica de optimización o, cuando procede, la geometría real de ida y vuelta. Como la geometría no se persiste, una descarga posterior puede regenerar la ruta; aunque las semillas son deterministas, el proveedor de rutas podría evolucionar. No hay aún otros formatos ni integración directa con Garmin o Strava.
