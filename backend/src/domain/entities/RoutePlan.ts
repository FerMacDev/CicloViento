export interface RoutePlanProps { id: string; userId: string; startLocation: string; date: string; distanceKm: number; elevationGainM: number; favorableWind: boolean; latitude: number; longitude: number; createdAt: Date; }
...
export class RoutePlan {
  ...
  static create(props: RoutePlanProps): RoutePlan {
    ...
    return new RoutePlan(props.id, props.userId, startLocation, props.date, props.distanceKm, props.elevationGainM, props.favorableWind, props.latitude, props.longitude, props.createdAt);
  }
}
