...
export class GenerateRouteGpxController {
  ...
  handle = async (request: Request, response: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (error instanceof NoViableWindRouteError || error instanceof RouteNotFoundError) { response.status(422).json({ message: 'No se ha encontrado un recorrido ciclista circular para esta planificación.' }); return; }
