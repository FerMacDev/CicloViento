...
import { RouteNotFoundError, type GeneratedRoute, type RoutingService } from '../src/application/services/RoutingService.js';
...
class Router implements RoutingService { readonly maximumRoundTripDistanceKm = 100; calls = 0; fallbackCalls = 0; failRoundTrip = false; async generateRoundTrip(): Promise<GeneratedRoute> { this.calls += 1; if (this.failRoundTrip) throw new RouteNotFoundError(); return this.route(); } async generateOutAndBack(): Promise<GeneratedRoute> { this.fallbackCalls += 1; return this.route(); } private route(): GeneratedRoute { return { start: { latitude: 40.48, longitude: -3.36 }, geometry: [{ latitude: 40.48, longitude: -3.36 }, { latitude: 40.49, longitude: -3.35 }, { latitude: 40.48, longitude: -3.36 }], distanceM: 78_900, durationS: 14_400, ascentM: 690 }; } }
...
test('GenerateCyclingRouteUseCase uses a real out-and-back fallback only after a missing round trip', async () => {
  const router = new Router();
  router.failRoundTrip = true;
  const result = await new GenerateCyclingRouteUseCase(new Repo(plan), router).execute('plan-1', 'user-1');

  assert.equal(router.calls, 1);
  assert.equal(router.fallbackCalls, 1);
  assert.equal(result.routeKind, 'out-and-back');
  assert.equal(result.fallbackReason, 'round-trip-unavailable');
  assert.deepEqual(result.route.geometry.at(-1), result.route.start);
});
