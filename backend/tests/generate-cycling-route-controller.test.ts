...
test('generate route controller exposes only the optimized route summary', async () => {
  const result = {
    ...
    optimization: {
      candidateCount: 2, selectedCandidate: 2,
      ...
      candidates: [{ seed: 1, actualDistanceKm: 49, favorableWindScore: 50, returnTailwindPercent: 50, returnHeadwindPercent: 40, returnCrosswindPercent: 10, selected: false }],
    },
  };
