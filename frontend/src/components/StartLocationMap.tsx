...
import type { WindRiskLevel } from './wind-direction';
...
export function StartLocationMap({latitude,longitude,startLocation,geometry=[],windDirectionDegrees,windRiskLevel,showWind=false}:{latitude:number;longitude:number;startLocation:string;geometry?:RouteCoordinate[];windDirectionDegrees?:number;windRiskLevel?:WindRiskLevel;showWind?:boolean}) {
  ...
  return <div className="route-map"><MapContainer center={[latitude,longitude]} zoom={12} style={{height:'320px'}}><TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/><Marker position={[latitude,longitude]} icon={icon}><Popup>Punto de partida: {startLocation}</Popup></Marker>{positions.length > 1 && <><Polyline positions={positions} pathOptions={{ color: '#176b87', weight: 5 }} /><FitRoute coordinates={geometry}/>{windDirectionDegrees !== undefined && windRiskLevel !== undefined && <WindDirectionLayer geometry={geometry} windDirectionDegrees={windDirectionDegrees} riskLevel={windRiskLevel} visible={showWind}/>}</>}</MapContainer></div>;
}
